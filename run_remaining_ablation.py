...
def check_experiment_done(experiment_name):
    """检查实验是否已完成"""
    checkpoint_path = os.path.join("checkpoints", experiment_name, "best_model.pt")
    return os.path.exists(checkpoint_path)

def load_existing_result(experiment_name, config):
    """加载已完成的实验结果"""
    checkpoint_path = os.path.join("checkpoints", experiment_name, "best_model.pt")
    if os.path.exists(checkpoint_path):
        import torch
...
def main():
    ...
    for exp in all_experiments:
        if check_experiment_done(exp["name"]):
            print(f"[已完成] {exp['name']}")
            completed_experiments.append(exp)
        else:
            print(f"[待运行] {exp['name']}")
            remaining_experiments.append(exp)
    
    print(f"\n已完成: {len(completed_experiments)} 个")
    print(f"待运行: {len(remaining_experiments)} 个")
    
    # 加载已完成实验的结果
    all_results = []
    for exp in completed_experiments:
        result = load_existing_result(exp["name"], exp["config"])
        if result:
            all_results.append(result)
            print(f"已加载: {exp['name']}")
    
    # 运行剩余实验
    if remaining_experiments:
        print(f"\n开始运行剩余 {len(remaining_experiments)} 个实验...")
        print("=" * 60)
